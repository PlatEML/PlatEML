...
def ps_tree_predict(
    pop_size, gen, tournament_size, data, operator_select, state, language
):
    gp = PSTreeRegressor(
    ...
    result.insert(
        ...
        "",
        [
            f"{translate[language]['Num']}",
            f"{translate[language]['Predicted_Val']}",
            f"{translate[language]['Actual_Val']}",
            f"{translate[language]['Bias']}",
        ],
    )
    ...
    data.close()
    base64_datas = [base64_data]
    train_infos = []
    state[0] = 100
    return result, base64_datas, train_infos, gp


def evolutionary_forest_predict(
    pop_size, gen, tournament_size, data, operator_select, state, language
):
...
def test_predict(data, gp, flag, language):
    ...
    if flag == 1:
        ...
        result.columns = [
            f"{translate[language]['Data']}" + str(c) for c in result.columns
        ]
        result.insert(
            0,
            "",
...
def scores_difference(
    ...
    regressor_list = [
        ...
        "AdaBoost",
        "GBDT",
    ]  # , 'DART', 'XGBoost', 'LightGBM', 'CatBoost'
    regressor_dict = {
