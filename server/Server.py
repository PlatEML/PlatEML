...
def description_card(session_id):
    ...
    return html.Div(
        ...
        children=[
            ...
            html.H6(
                translate[global_dict[session_id]["language"]][
...
def get_option_bar(task, session_id):
    ...
    if task is None or task == "" or task == "interpretable-modeling":
        return [
            html.P(
                children=translate[global_dict[session_id]["language"]][
                    "Tournament_Size"
                ],
                style={"font-weight": "bold"},
            ),
            ...
            html.Br(),
            html.P(
                children=translate[global_dict[session_id]["language"]]["Operators"],
                style={"font-weight": "bold"},
            ),
            dcc.Dropdown(
                id="operator-select",
            ...
            html.Div(
                dcc.Input(id="moea-operator-select", value=""),
                style={"display": "none"},
            ),
        ]
        ...
        return [
            html.Div(
                dcc.Input(id="tournament_size", value=""), style={"display": "none"}
            ),
            html.Div(
                dcc.Input(id="operator-select", value=""), style={"display": "none"}
            ),
            html.Div(
                dcc.Input(id="moea-operator-select", value=""),
                style={"display": "none"},
...
def generate_control_card(session_id):
    ...
    return html.Div(
        ...
        children=[
            html.P(
                children=translate[global_dict[session_id]["language"]]["Training_Data"]
            ),
            ...
            html.Br(),
            html.P(
                children=translate[global_dict[session_id]["language"]]["Task_Type"]
            ),
            html.Div(
                dcc.Dropdown(
                    id="task-type",
            ...
            html.Div(
                ...
                children=html.Button(
                    id="start-btn",
                    children=translate[global_dict[session_id]["language"]]["Start_up"],
                    n_clicks=0,
                ),
            ),
    ...
    main_page = [
        html.Div(
            ...
            [
                ...
                # ),
                html.H4(
                    children=f"ECNU {translate[global_dict[session_id]['language']]['Interpretable_Machine']}",
                ...
                html.Div(
                    ...
                    children=[
                        description_card(session_id),
                        generate_control_card(session_id),
                    ]
                    + [
                        html.Div(
                            ["initial child"],
                    ...
                    children=[
                        html.Div(
                            ...
                            children=[
                                html.B(
                                    children=translate[
                                        global_dict[session_id]["language"]
                                    ]["Training_Data"]
                                ),
                                html.Hr(),
    ...
    if n_clicks > 0:
        ...
        if task_type in ["interpretable-modeling", "evolutionary-forest", "PS-Tree"]:
            ...
            if task_type == "interpretable-modeling":  # gplearn
                data, base64_datas, train_infos, gp = gp_predict(
                    pop_size,
                    generation,
                    tournament_size,
                    global_data,
                ...
                data, base64_datas, train_infos, gp = ps_tree_predict(
                    pop_size,
                    generation,
                    tournament_size,
                    global_data,
                ...
                data, base64_datas, train_infos, gp = evolutionary_forest_predict(
                    pop_size,
                    generation,
                    tournament_size,
                    global_data,
            ...
            if task_type == "PS-Tree":
                return (
                    generate_table(data.T, session_id),
                    html.Div(
                        [
                            html.Li(
                                html.A(
                                    ...
                                    href=f"/download/{str(session_id)}r.csv",
                                    target="_blank",
                                )
                            )
                        ]
                ...
                names, importance = list(code_importance_dict.keys()), list(
                    code_importance_dict.values()
                )
            ...
            pf_data, pf_figure = feature_selection_utils(
                pop_size,
                generation,
                global_data,
                moea_operator_select,
            ...
            global_dict[session_id]["training_infos"] = training_infos
            return (
                generate_table(pf_data, session_id, 2),
                html.Div(
                    [
                        html.Li(
                            html.A(
...
def render_image(model_id, session_id):
    ...
    data, base64_datas, train_infos = (
        training_infos["data"],
        training_infos["base64_datas"],
    ...
    return [
        ...
        html.Img(
            src="data:image/png;base64,{}".format(base64_datas[model_id]),
            style={
                "display": "block",
                "margin-left": "auto",
                "margin-right": "auto",
...
@app.callback(
    Output("training-state", "children"),
    [Input("upload_data", "isCompleted")],
    [State("upload_data", "fileNames"), State("session-id", "children")],
)
def update_output(isCompleted, fileNames, session_id):
    ...
    if isCompleted:
        ...
        for i in range(0, df.shape[1]):
            if (empty_data[i] == True) or (
                number_data[i] != "float64" and number_data[i] != "int64"
            ):
                return html.Div(
                    children=translate[global_dict[session_id]["language"]][
                        "Error_numeric"
    ...
    df.columns = [
        f"{translate[global_dict[session_id]['language']]['Variable']}" + str(c)
        if i < len(df.columns) - 1
        else f"{translate[global_dict[session_id]['language']]['Target_Variable']}"
        for i, c in enumerate(df.columns)
    ]
    df.insert(
        0,
        f"{translate[global_dict[session_id]['language']]['Number']}",
        np.arange(0, len(df)),
    )
...
@app.callback(
    [
        Output("test-state", "children"),
        Output("download-test", "children"),
    ],
    [Input("upload_testdata", "isCompleted")],
    [State("upload_testdata", "fileNames"), State("session-id", "children")],
)
def update_test(isCompleted, fileNames, session_id):
    if isCompleted:
        ...
        data = test_predict(
            df,
            global_dict[session_id]["training_infos"]["gp"],
            flag,
            global_dict[session_id]["language"],
        )
        # 每次新上传数据，删掉之前ef训练的特征
        ...
        return generate_table(data.T, session_id, 1, 3), html.Div(
            [
                html.Li(
                    html.A(
                        children=translate[global_dict[session_id]["language"]][
                        ...
                        href=f"/download/{str(session_id)}t.csv",
                        target="_blank",
                    )
                )
            ]
...
@app.callback(
    ...
    [State("check-list-input", "value"), State("session-id", "children")],
)
def check_list_output(clicks, value, session_id):
    if clicks > 0:
        ...
        new_train = feature_append(
            r,
            x_train,
            code_importance_dict[: len(code_importance_dict)],
            only_new_features=True,
        )
        new_test = feature_append(
            r,
            x_test,
            code_importance_dict[: len(code_importance_dict)],
            only_new_features=True,
        ...
        scores_difference(
            x_train, x_test, new_train, new_test, y_train, y_test, session_id
        )
        ...
        return [
            html.H4(
                children=translate[global_dict[session_id]["language"]][
                    "Effect_of_Feature_Construction"
                ],
                style={"textAlign": "center", "font-size": "2.0rem"},
            ),
            html.Img(
                src="data:image/png;base64,{}".format(base64_data1),
                style={
                    "display": "block",
                    "margin-left": "auto",
                    "margin-right": "auto",
